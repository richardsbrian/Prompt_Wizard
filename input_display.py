...
def initialize_cancel_button():
    ...
    button_rect.topright = (pygame.display.get_surface().get_width() - 20, 20)  # Positioning it at the top right corner
...
def render(screen, screenshot_image, input_box, color, text, font, title, title_font, cancel_button, button_color):
    screen.fill((30, 30, 30))

    # Center the image
    img_rect = screenshot_image.get_rect(center=(screen.get_width() // 2, screen.get_height() // 2))
    screen.blit(screenshot_image, img_rect.topleft)

    # Render the title
    title_surface = title_font.render(title, True, pygame.Color('white'))
    title_rect = title_surface.get_rect(center=(screen.get_width() // 2, input_box.y - 40))
...
def display_screenshot_with_input(filename):
    pygame.init()
    screen = pygame.display.set_mode((0, 0), pygame.FULLSCREEN)  # Set the screen to full screen
    pygame.display.set_caption("Screenshot with Input Box")
    screenshot_image = pygame.image.load(filename)
    input_box, color_inactive, color_active = initialize_input_box(screen.get_width(), screen.get_height())
    ...
    title = "Enter your text below:"  # Define the title text
    ...
    while True:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                return False
            active, text, finished, cancelled = handle_event(event, input_box, active, text, cancel_button)
            if finished:
                return True
            if cancelled:
                return False
        ...
        render(screen, screenshot_image, input_box, color, text, font, title, title_font, cancel_button, button_color)
        clock.tick(30)